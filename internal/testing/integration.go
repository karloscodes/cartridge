...
func CleanupTestDB(db interface{}) error {
	// TODO: This would clean all tables when GORM is available
...
func (c *IntegrationTestClient) WithCleanDatabase(testFunc func()) {
	...
	db := c.app.GetDatabase().GetGenericConnection()
	if db == nil {
	...
	// Clean up after test
	defer func() {
		if err := CleanupTestDB(c.app.GetDatabase()); err != nil {
