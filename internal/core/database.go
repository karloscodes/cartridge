...
// sqliteDatabase implements DatabaseManager for SQLite
...
func NewDatabaseManager(cfg Config, logger Logger) DatabaseManager {
	dbOnce.Do(func() {
		dbInstance = &sqliteDatabase{
			config: cfg,
			logger: logger,
...
func GetDatabaseInstance() DatabaseManager {
	if dbInstance == nil {
		panic("Database manager not initialized. Call NewDatabaseManager first.")
	}
	return dbInstance
}
...
func (dm *sqliteDatabase) Init() error {
	...
	dm.logger.Info("Initializing database connection", "url", dm.config.GetDatabaseURL())
	...
	db, err := gorm.Open(sqlite.Open(dm.config.GetDatabaseURL()), &gorm.Config{
		Logger: gormLogger,
	})
	if err != nil {
		dm.logger.Error("Failed to connect to database", "error", err, "url", dm.config.GetDatabaseURL())
